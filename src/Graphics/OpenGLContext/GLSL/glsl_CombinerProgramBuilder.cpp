...
public:
	...
	{
		if (_glinfo.isGLES2) {
			static const std::string strReadTex0Gles2 =
				...
				;
			static const std::string strReadTex1Gles2 =
				...
				"  mediump vec4 texWdthAndOff = 255.0 * texture2D(tex, vec2(tile + 0.5, 0.5)/texSize);	\n"
				...
				"  mediump vec2 Coords00 = vec2(tcData[0] * lod_scale);									\n"
				"  mediump float offset00 = offset + width * Coords00.t + Coords00.s;					\n"
				"  mediump float Y00 = offset00 / mipmapTileWidth;										\n"
				"  lowp vec4 c00 = texture2D(tex, vec2(offset00 - mipmapTileWidth * Y00, Y00)/texSize);	\n"
				"  mediump vec2 Coords01 = vec2(tcData[1] * lod_scale);									\n"
				"  mediump float offset01 = offset + width * Coords01.t + Coords01.s;					\n"
				"  mediump float Y01 = offset01/mipmapTileWidth;										\n"
				"  lowp vec4 c01 = texture2D(tex, vec2(offset01 - mipmapTileWidth * Y01, Y01)/texSize);	\n"
				"  mediump vec2 Coords10 = vec2(tcData[2] * lod_scale);									\n"
				"  mediump float offset10 = offset + width * Coords10.t + Coords10.s;					\n"
				"  mediump float Y10 = offset10/mipmapTileWidth;										\n"
				"  lowp vec4 c10 = texture2D(tex, vec2(offset10 - mipmapTileWidth * Y10, Y10)/texSize);	\n"
				"  mediump vec2 Coords11 = vec2(tcData[3] * lod_scale);									\n"
				"  mediump float offset11 = offset + width * Coords11.t + Coords11.s;					\n"
				"  mediump float Y11 = offset11/mipmapTileWidth;										\n"
				"  lowp vec4 c11 = texture2D(tex, vec2(offset11 - mipmapTileWidth * Y11, Y11)/texSize);	\n"
				;
			...
			if (config.texture.bilinearMode == BILINEAR_3POINT) {
				m_part = strReadTex0Gles2;
				m_part += strBillinear3PointEndGles2;
				m_part += strReadTex1Gles2;
				m_part += strBillinear3PointEndGles2;
			} else {
				m_part = strReadTex0Gles2;
			...
			m_part +=
				...
				"  else readtex0 = TextureMipMap1(uTex1, tcData1, tile0 - 1);				\n"
				"  if (tile1 == 0)															\n"
				"    readtex1 = TextureMipMap0(uTex0, tcData0);								\n"
				"  else if (uEnableLod == 0 || uMaxTile == 1)								\n"
				"    readtex1 = TextureMipMap0(uTex1, tcData1);								\n"
				"  else readtex1 = TextureMipMap1(uTex1, tcData1, tile1 - 1);				\n"
				"  return lod_frac;															\n"
				"}																			\n"
				;
		}
